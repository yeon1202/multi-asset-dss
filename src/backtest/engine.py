...
RebalanceFreq = Literal["M", "Q", "W", "D"]  # 월말 / 분기말 / 주말 / 매일
...
def _max_consecutive_nan(s: pd.Series) -> int:
    """한 컬럼의 최대 연속 NaN 길이 — 상장폐지/거래정지 감지용."""
    nan_mask = s.isna()
    if not nan_mask.any():
        return 0
    runs = (nan_mask != nan_mask.shift()).cumsum()
    return int(nan_mask.groupby(runs).sum().max())
...
def run_backtest(
    ...
    max_consecutive_nan_warn: int = 5,
) -> BacktestResult:
    """
    Parameters
    ----------
    ...
    # 결측 진단 — 연속 결측 길어지면 상장폐지/거래정지 의심
    for col in prices.columns:
        consec = _max_consecutive_nan(prices[col])
        if consec > max_consecutive_nan_warn:
            logger.warning(
                f"⚠️ {col}: 연속 {consec}일 결측 — 상장폐지/거래정지 가능성. "
                "ffill 로 채워서 진행하지만 결과 왜곡 주의."
            )
    ...
    for date in prices.index:
        ...
        # ⚠️ Look-ahead bias 방지 (PROJECT_SPEC.md §7.2):
        # 전략에는 "어제 종가까지" (= iloc[:-1]) 만 보여주고, 체결은 오늘 종가에서.
        # 현실에서는 어제 종가를 보고 오늘 시가/종가에 주문이 체결되므로 이게 맞음.
        if date in rebalance_days and portfolio_value > 0:
            history = prices.loc[:date].iloc[:-1]   # 어제까지만
